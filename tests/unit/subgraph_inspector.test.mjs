...
// ── 1. SUPER SUBGRAPH = SUBGRAFO NATIVO: entrar usa a navegação nativa ──
fakeCanvas.openSubgraph = function (sub, node) { this.graph = sub; this.openedFrom = node; };
const nativeInner = new FakeGraph();
nativeInner.isRootGraph = false;
const sn = {
  id: 101, type: "4fc0e6fd-0000-4000-8000-000000000000", title: "Super1",
  subgraph: nativeInner, isSubgraphNode: () => true,
  properties: { ui_layout: { schema: 2, tabs: [{ sections: [{ controls: [] }] }] } },
  flags: {}, widgets: [], setSize() {},
  addDOMWidget(name, type, el, opts) { const w = { name, type, element: el, ...opts }; this.widgets.push(w); document.body.append(el); return w; },
};
M.attach(sn);
M.enterSuper(sn);
t("enterSuper opens the native subgraph (openSubgraph)", fakeCanvas.graph === nativeInner && fakeCanvas.openedFrom === sn);
fakeCanvas.graph = rootGraph;
