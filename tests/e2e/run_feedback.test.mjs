...
const mode1 = await E((ids) => ({ btn: document.querySelector('.lego-row[data-name="Stepper1"] .lego-seed-mode')?.textContent, w: window.app.graph.getNodeById(ids.sn).__ssGraph.getNodeById(ids.ni).widgets.find(w => w.options?.values?.includes?.("randomize")).value }), ids);
...
await E((ids) => { window.app.graph.getNodeById(ids.sn).__ssGraph.getNodeById(ids.li).widgets[0].value = "missing_" + Date.now() + ".png"; }, ids);
await E(async () => { await window.app.queuePrompt(0, 1); });
await pg.waitForFunction(() => !!document.querySelector(".lego-run-error"), null, { timeout: 30000 }).catch(() => {});
...
const seed = await E((ids) => window.app.graph.getNodeById(ids.sn).__ssGraph.getNodeById(ids.ni).widgets.find(w => w.name === "value").value, ids);
